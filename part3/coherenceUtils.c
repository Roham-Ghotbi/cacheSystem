...
enum state determineState(cache_t* cache, uint32_t address) {
	...
	uint8_t isDirty = getDirty(cache, blockInfo->blockNumber);
	uint8_t isShared = getShared(cache, blockInfo->blockNumber);
	uint8_t isValid = getValid(cache, blockInfo->blockNumber);
	bool isMatch = blockInfo->match;
	free(blockInfo);
...
void updateState(cache_t* cache, uint32_t address, enum state otherState) {
	//Roham
	evictionInfo_t *blockInfo = findEviction(cache,address);
	uint32_t tag = getTag(cache, blockInfo->blockNumber);
	uint32_t blockIndex= getIndex(cache, address);
	long oldLRU = getLRU(cache, blockInfo->blockNumber);

	enum state initialState = determineState(cache, address);

	if(initialState == MODIFIED) {							//if B is Modified

		if(otherState == SHARED) {
			setState(cache, blockInfo->blockNumber, OWNED);
		} else if(otherState == MODIFIED) {
			setState(cache, blockInfo->blockNumber, INVALID);
		}

	} else if (initialState == EXCLUSIVE) {					//if B is Exclusive

		if(otherState == SHARED) {
			setState(cache, blockInfo->blockNumber, SHARED);
		} else if (otherState == MODIFIED) {
            setState(cache, blockInfo->blockNumber, INVALID);
        }

	} else if (initialState == OWNED) {						//if B is Owned

		if (otherState == MODIFIED) {
			setState(cache, blockInfo->blockNumber, INVALID);
		} else if (otherState == EXCLUSIVE || otherState == INVALID) {
			setState(cache, blockInfo->blockNumber, MODIFIED);
		}

	} else if (initialState == SHARED) {					//if B is Shared

		if (otherState == MODIFIED) {
			setState(cache, blockInfo->blockNumber, INVALID);
		} else if (otherState == EXCLUSIVE || otherState == INVALID) {
			setState(cache, blockInfo->blockNumber, EXCLUSIVE);
		}

	} else if (initialState == INVALID) {					//if B is Invalid

		if (otherState == SHARED || otherState == OWNED) {
			setState(cache, blockInfo->blockNumber, INVALID);
		}
	}
	decrementLRU(cache, tag, blockIndex, oldLRU);
	free(blockInfo);
	//ENDRoham
}
